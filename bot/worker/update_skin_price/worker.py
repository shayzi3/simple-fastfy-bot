...
from .base import UpdateSkinPriceBaseWorker


class UpdateSkinPriceWorker(UpdateSkinPriceBaseWorker):
     def __init__(self):
          super().__init__()
     
     
     async def run(self) -> None:
          logging_.worker_update_prices.info("UPDATE SKIN PRICE START")
          
          functions = [
               self.__update_skin_price(
     ...
     async def __update_skin_price(
          self, 
          mode: SkinUpdateMode, 
          sleep: timedelta
     ) -> None:
          logging_.worker_update_prices_at_days.info(f"MODE {mode} START")
          
          await asyncio.sleep(sleep.total_seconds())
          await self._process(mode=mode)
          await self.__update_skin_price(mode=mode, sleep=sleep)
